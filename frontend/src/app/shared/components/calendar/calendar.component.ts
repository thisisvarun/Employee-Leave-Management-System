...
export class ZardCalendarComponent {
  ...
  protected readonly weekdayClasses = computed(() =>
    mergeClasses(calendarWeekdayVariants({ zSize: this.zSize() }))
  );

  protected readonly dayContainerClasses = computed(() =>
    mergeClasses(calendarDayVariants({ zSize: this.zSize() }))
  );
  ...
  protected onMonthChange(monthIndex: string): void {
    ...
    const newDate = new Date(
      isNaN(selectedYear) ? currentDate.getFullYear() : selectedYear,
      parsedMonth,
      1
    );
  ...
  protected onYearChange(year: string): void {
    ...
    const newDate = new Date(
      parsedYear,
      isNaN(selectedMonth) ? currentDate.getMonth() : selectedMonth,
      1
    );
  ...
  protected isPreviousDisabled(): boolean {
    ...
    const lastDayOfPreviousMonth = new Date(
      currentDate.getFullYear(),
      isNaN(currentMonth) ? currentDate.getMonth() : currentMonth,
      0
    );
  ...
  selectDate(date: Date, i?: number) {
    ...
    this.value.set(date);
    this.focusedDayIndex.set(
      i ?? this.calendarDays().findIndex((day) => this.isSameDay(day.date, date))
    );
  }
  ...
  private setFocus(index: number): void {
    ...
    setTimeout(() => {
      const dayElement = this.calendarContainer()?.nativeElement.querySelector(
        `#${this.getDayId(index)}`
      ) as HTMLElement;
      dayElement?.focus();
    }, 0);
